...
class SoftTreeClassifier:
    ...
    def __split_pair(self, X, y, candidate_features):
        ...
        for feat in candidate_features:
            ...
            if attr == "d" or unique_col.shape == 1:
                ...
                threshold_list = [
                    (unique_col[i] + unique_col[i + 1]) / 2
                    for i in range(len(unique_col) - 1)
                ]

            for t in threshold_list:
                true_index, false_index = self.__split(X, feat, t)
                p = float(len(true_index)) / len(X)
                next_gini = p * soft_gini(y[true_index]) + (1 - p) * soft_gini(
                    y[false_index]
                )
    ...
    def __build_tree(self, X, y):
        ...
        candidate_features = np.random.choice(
            candidate_features,
            min(self.n_features, len(candidate_features)),
            replace=False,
        )
