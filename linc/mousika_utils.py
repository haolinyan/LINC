...
def split_train_test(df, train_percent=0.8, bin=True):
    ...
    for col_names in ["dstAddr{}".format(i) for i in range(1, 5)]:
        df[col_names] = df[col_names].astype("str")
    df["srcIP"] = df["srcAddr1"].str.cat(
        [df["srcAddr2"], df["srcAddr3"], df["srcAddr4"]], sep="."
    )
    df["dstIP"] = df["dstAddr1"].str.cat(
        [df["dstAddr2"], df["dstAddr3"], df["dstAddr4"]], sep="."
    )
    ...
    for key in label_dic:
        if label_dic[key] > label_dic[winner_key]:
            ...
            if voting_rule == "random":
                winner_key = np.random.choice([key, winner_key], 1)[
                    0
                ]  # return a list with len 1
...
def set_bit_val(byte, index, val, total_bit=112):
    """
    set one bit of byte
...
class Rule2Binary:
    ...
    def get_ternary(self, feature_list, tmp_cls, total_bit=112):
        """
        ...
        for feature_index, equal_str, feature_value in feature_list:
            tmp_mask = set_bit_val(tmp_mask, int(feature_index), 1, total_bit=total_bit)
            if "!" in equal_str:
                tmp_value = set_bit_val(
                    tmp_value,
                    int(feature_index),
                    int(feature_value) ^ 1,
                    total_bit=total_bit,
    ...
    def parse_file(self, file_path, total_bit=112):
        """
        ...
        with open(file_path, "r", encoding="utf-8") as f:
            for line in f:
                ...
                ternary_list.append(
                    self.get_ternary(matchObj, tmp_cls, total_bit=total_bit)
                )
        return ternary_list
...
def rule2entry(input_file, total_bit=112):
    save_file = "tmp_rule2entry.txt"
    if os.path.exists(save_file):
        # delete the file
        os.remove(save_file)
    rule2binaly = Rule2Binary()
    ternary_list = rule2binaly.parse_file(input_file, total_bit=total_bit)
