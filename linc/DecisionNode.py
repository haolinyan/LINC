...
class DecisionNode:
        ...
        if self.label is not None:
            ...
            if feature_attr[self.feature] == "c":
                ...
                if data_vec[self.feature] < self.threshold:
                    path[-1].append(-1)
                    return self.false_branch.find_path(
                        data_vec, path, feature_attr, attr_map
                    )
                else:
                    path[-1].append(1)
                    return self.true_branch.find_path(
                        data_vec, path, feature_attr, attr_map
                    )
            else:
                path.append(
                    [
                        self.feature + 1,
                        attr_map[str(self.feature) + "_" + str(int(self.threshold))],
                        attr_map[
                            str(self.feature) + "_" + str(int(data_vec[self.feature]))
                        ],
                    ]
                )
                if data_vec[self.feature] != self.threshold:
    ...
    def show_tree(self, path, feature_attr, attr_map, model_path):
        if self.label is not None:
            ...
            if feature_attr[self.feature] == "c":
                path.append(
                    " if feature_" + str(self.feature) + "<" + str(self.threshold)
                )
                # 左递归
